angular.module("drupal_web.controllers", [])
...
.controller("side_menusCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/drupal_web/about_us\" ng-click=\"popover.hide()\">";
...
// TODO: nodeCtrl --|-- 
.controller("nodeCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$rootScope.grid128 = parseInt($rootScope.ionWidth / 128) ;
	$rootScope.grid256 = parseInt($rootScope.ionWidth / 256) ;
	$rootScope.last_edit = "table (node)" ;
	$scope.$on("$ionicView.afterEnter", function (){
		var page_id = $state.current.name ;
		$rootScope.page_id = page_id.replace(".","-") ;
	});
	...
	// TODO: nodeCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: nodeCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: nodeCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: nodeCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: nodeCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: nodeCtrl --|-- $scope.showAuthentication
	...
	// TODO: nodeCtrl --|-- $scope.splitArray
	...
	// TODO: nodeCtrl --|-- $scope.addToVirtual
	$scope.addToDbVirtual = function(newItem){
		var is_already_exist = false ;
		// animation loading 
		$ionicLoading.show();
		var virtual_items = []; 
		localforage.getItem("node_bookmark", function(err,dbVirtual){
			if(dbVirtual === null){
				virtual_items = [];
			}else{
				try{
			...
			localforage.setItem("node_bookmark",JSON.stringify(virtual_items));
			$timeout(function(){
				$ionicLoading.hide();
			},200);
		}).catch(function(err){
	...
	// TODO: nodeCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "http://demo.ihsana.net/drupal/api/node";
	// TODO: nodeCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "http://demo.ihsana.net/drupal/api/node?callback=JSON_CALLBACK";
	// TODO: nodeCtrl --|-- $scope.hashURL
	...
	localforage.getItem("data_nodes_" + $scope.hashURL, function(err, get_nodes){
		if(get_nodes === null){
			data_nodes =[];
		}else{
			data_nodes = JSON.parse(get_nodes);
			$scope.data_nodes =JSON.parse( get_nodes);
			$scope.nodes = [];
			for(lastPush = 0; lastPush < 50; lastPush++) {
				if (angular.isObject(data_nodes[lastPush])){
					$scope.nodes.push(data_nodes[lastPush]);
				};
			}
			$timeout(function() {
				$ionicLoading.hide();
	...
	if(data_nodes.length === 0 ){
		$timeout(function() {
			...
			$http.get(url_request,http_header).then(function(response) {
				data_nodes = response.data;
				console.log("%cSuccessfully","color:blue;font-size:18px");
				console.dir(data_nodes);
				$scope.data_nodes = response.data;
				// TODO: nodeCtrl --|---------- set:localforage
				localforage.setItem("data_nodes_" + $scope.hashURL, JSON.stringify(data_nodes));
				$scope.nodes = [];
				for(lastPush = 0; lastPush < 50; lastPush++) {
					if (angular.isObject(data_nodes[lastPush])){
						$scope.nodes.push(data_nodes[lastPush]);
				...
				$timeout(function() {
					...
					if(response.status ===401){
						// TODO: nodeCtrl --|------------ error:Unauthorized
						$scope.showAuthentication();
					}else{
						// TODO: nodeCtrl --|------------ error:Message
	...
	// TODO: nodeCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");
			...
			console.log("%cerror: %cPage: `node` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: node_bookmarkCtrl --|-- 
.controller("node_bookmarkCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: node_bookmarkCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: node_bookmarkCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: node_bookmarkCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: node_bookmarkCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: node_bookmarkCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: node_bookmarkCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: node_bookmarkCtrl --|-- Database for Cart/Bookmark
	// TODO: node_bookmarkCtrl --|-- $scope.loadDbVirtualNode
	 
	$scope.loadDbVirtualNode = function(){
		$ionicLoading.show();
		//node_bookmark
		$scope.node_bookmark = []; 
		localforage.getItem("node_bookmark", function(err,dbVirtual){
			if(dbVirtual === null){
				...
				try{
					$scope.node_bookmark = JSON.parse(dbVirtual); 
				}catch (e){
					$scope.node_bookmark = []; 
				}
			}
	...
	$scope.$on("$ionicView.enter", function (){
		$scope.loadDbVirtualNode();
	});
	// TODO: node_bookmarkCtrl --|-- $scope.clearDbVirtualNode
	$scope.clearDbVirtualNode = function(){
		localforage.setItem("node_bookmark",[]);
		$timeout(function(){
			$scope.loadDbVirtualNode();
		},200);
	};
	// TODO: node_bookmarkCtrl --|-- $scope.removeDbVirtualNode
	$scope.removeDbVirtualNode = function(itemId){
		var virtual_items = [];
		var last_items = $scope.node_bookmark;
		for(var z=0;z<last_items.length;z++){
			if(itemId!==last_items[z].nid){
				virtual_items.push(last_items[z]);
			}
		}
		localforage.setItem("node_bookmark",JSON.stringify(virtual_items));
	...
	// TODO: node_bookmarkCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
			...
			console.log("%cerror: %cPage: `node_bookmark` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: node_singlesCtrl --|-- 
.controller("node_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: node_singlesCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: node_singlesCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: node_singlesCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: node_singlesCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: node_singlesCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: node_singlesCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: node_singlesCtrl --|-- $scope.showAuthentication
	$scope.showAuthentication  = function(){
		var authPopup = $ionicPopup.show({
			template: ' This page required login',
			title: "Authorization",
	...
	// TODO: node_singlesCtrl --|-- $scope.addToVirtual(); //data single
	$scope.addToDbVirtual = function(newItem){
		var is_already_exist = false ;
		// animation loading 
		$ionicLoading.show();
		...
		localforage.getItem("node_bookmark", function(err,dbVirtual){
			if(dbVirtual === null){
				...
				for(var z=0;z<last_items.length;z++){
					virtual_items.push(last_items[z]);
					if(newItem.nid ==  last_items[z].nid){
						is_already_exist = true;
					}
				}
			}
	...
	// Retrieving data
	var itemID = $stateParams.nid;
	// TODO: node_singlesCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "http://demo.ihsana.net/drupal/api/node/" + itemID;
	// TODO: node_singlesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "http://demo.ihsana.net/drupal/api/node/" + itemID + "?callback=JSON_CALLBACK";
	// TODO: node_singlesCtrl --|-- $scope.hashURL
	$scope.hashURL = md5.createHash($scope.fetchURL);
	
	var current_item = [];
	localforage.getItem("data_node_single_" + $scope.hashURL, function(err, get_datas){
		if(get_datas === null){
			current_item = [];
		}else{
			if(get_datas !== null){
	...
	if( current_item.length === 0 ){
		...
		// TODO: node_singlesCtrl --|-- $http.get
		$http.get($scope.fetchURL,http_header).then(function(response) {
			// Get data single
			var datas = response.data;
			// TODO: node_singlesCtrl --|---------- set:localforage
			localforage.setItem("data_node_single_" + $scope.hashURL,JSON.stringify(datas));
			current_item = datas ;
		},function(data) {
					// Error message
					var alertPopup = $ionicPopup.alert({
		...
		// TODO: node_singlesCtrl --|-- $scope.doRefresh
	$scope.doRefresh = function(){
		// Retrieving data
		var itemID = $stateParams.nid;
		var current_item = [];
		...
		$http.get($scope.fetchURL,http_header).then(function(response) {
			...
			// Error message
		// TODO: node_singlesCtrl --|---------- $http.jsonp
	...
	// TODO: node_singlesCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `node_singles` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
